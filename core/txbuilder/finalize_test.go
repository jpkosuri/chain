...
func TestSighashCheck(t *testing.T) {
	...
	_, err = issue(ctx, t, info, g, info.acctA.ID, 10)
	if err != nil {
		t.Fatal(err)
	}
	_, err = issue(ctx, t, info, g, info.acctB.ID, 10)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTransferConfirmed(t *testing.T) {
	...
	_, err = transfer(ctx, t, info, g, info.acctA.ID, info.acctB.ID, 10)
	if err != nil {
		testutil.FatalErr(t, err)
	}
}
...
func BenchmarkTransferWithBlocks(b *testing.B) {
	...
	g := generator.New(info.Chain, nil, db)
	for i := 0; i < b.N; i++ {
		tx, err := issue(ctx, b, info, g, info.acctA.ID, 10)
		if err != nil {
			b.Fatal(err)
		}
		...
		tx, err = transfer(ctx, b, info, g, info.acctA.ID, info.acctB.ID, 10)
		if err != nil {
			b.Fatal(err)
		}
		b.Logf("finalized %v", tx.Hash)
